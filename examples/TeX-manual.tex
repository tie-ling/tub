...
%
\documentclass[draft,leqno,titlepage,openany,a5paper]{amsldoc}[1999/12/13]
\usepackage[
left=1cm,right=1cm,top=2cm,bottom=1cm
]{geometry}
...
\let\indexfont\texttt
\makeatother
%    For a package that won't be indexed, i.e. amsmath,mathtools,fontspec
\newcommand{\nipkg}{\textsf}
\def\tikz/{Ti\textit{k}Z}
%    End definitions for amsldoc.cls

\title{User's Guide for the \nipkg{amsmath,mathtools,fontspec} Package (Version~2.1)}
...
%    Use the amsmath,mathtools,fontspec package and amscd package in order to print
%    examples.

\usepackage{amsmath,mathtools,fontspec,amssymb,parskip,interval,fontspec,mathtools}
...
\index{amsmath,mathtools,fontspec@\texttt{amsmath} package|(}

The \nipkg{amsmath,mathtools,fontspec} package is a \LaTeX{} package that provides
...
up-to-date version of \LaTeX{}, the \nipkg{amsmath,mathtools,fontspec} package is normally
provided along with it. Upgrading when a newer version of the
\nipkg{amsmath,mathtools,fontspec} package is released can be done via
\url{http://mirror.ctan.org/macros/latex/required/amsmath,mathtools,fontspec.zip}.

This documentation describes the features of the \nipkg{amsmath,mathtools,fontspec} package
...
in this list of \nipkg{amsmath,mathtools,fontspec} features:
\begin{itemize}
...
The \nipkg{amsmath,mathtools,fontspec} package is distributed together with some small
auxiliary packages:
\begin{description}
\item[\nipkg{amsmath,mathtools,fontspec}] Primary package, provides various features for
...
The \nipkg{amsmath,mathtools,fontspec} package incorporates \pkg{amstext}, \pkg{amsopn}, and
...
enhancements to \nipkg{amsmath,mathtools,fontspec}; \pkg{mathtools} loads \nipkg{amsmath}
automatically, so there is no need to separately load \nipkg{amsmath,mathtools,fontspec}
if \pkg{mathtools} is used.  Some \pkg{mathtools} facilities
will be noted below as appropriate.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Options for the \nipkg{amsmath,mathtools,fontspec} package}\label{options}

The \nipkg{amsmath,mathtools,fontspec} package has the following options:
\index{options!amsmath,mathtools,fontspec@\texttt{amsmath} package options|(}
\begin{description}
...
\verb"\usepackage[intlimits]{amsmath,mathtools,fontspec}".
...
The \nipkg{amsmath,mathtools,fontspec} package also recognizes the following options which
...
list of the \cn{usepackage}|{amsmath,mathtools,fontspec}| statement.
\begin{description}
...
The \nipkg{amsmath,mathtools,fontspec} package provides a number of additional displayed
...
mark alignment points. Unlike the other \nipkg{amsmath,mathtools,fontspec} equation
...
P_{r-j}=
  ...
  \end{cases}
\end{equation}
and in the \nipkg{amsmath,mathtools,fontspec} package there is a \env{cases} environment to
...
displays. The environments of the \nipkg{amsmath,mathtools,fontspec} package try hard to
...
space between lines in all the \nipkg{amsmath,mathtools,fontspec} displayed equation
...
defined by \nipkg{amsmath,mathtools,fontspec} the space is interpreted to mean that the
bracketed material is part of the document content.

When the \nipkg{amsmath,mathtools,fontspec} package is
...
the \nipkg{amsmath,mathtools,fontspec} package provides a
...
The \nipkg{amsmath,mathtools,fontspec} package provides also a wrapper environment,
...
The \nipkg{amsmath,mathtools,fontspec} package provides some environments for
...
The \nipkg{amsmath,mathtools,fontspec} package slightly extends the set of math
...
\nipkg{amsmath,mathtools,fontspec} will output the most suitable form based on the
...
accents is often poor. With the \nipkg{amsmath,mathtools,fontspec} package you
...
\nipkg{amsmath,mathtools,fontspec} package \cn{leftroot} and \cn{uproot} allow you to adjust
...
by the \nipkg{amsmath,mathtools,fontspec} package to extend the set:
...
In the \nipkg{amsmath,mathtools,fontspec} package there are somewhat more general commands,
...
\nipkg{amsmath,mathtools,fontspec} package provides also \cn{dfrac} and \cn{tfrac} as
...
\nipkg{amsmath,mathtools,fontspec} has \cn{binom}, \cn{dbinom} and \cn{tbinom}:
\begin{equation}
...
warning  messages if used with the \nipkg{amsmath,mathtools,fontspec} package, for reasons
...
symbols. With the \nipkg{amsmath,mathtools,fontspec} package \cn{smash} has optional
...
sizes.  With the \nipkg{amsmath,mathtools,fontspec} package they are.%
\index{delimiters!fixed size|)}

\subsection{Vertical bar notations}

The \nipkg{amsmath,mathtools,fontspec} package provides commands \cn{lvert}, \cn{rvert},
...
\nipkg{amsmath,mathtools,fontspec} package provides a general mechanism for defining new
...
\nipkg{amsmath,mathtools,fontspec} \cn{text} definition, but ensure the \verb+\rmfamily+
font is used.
\begin{equation}
...
The \nipkg{amsmath,mathtools,fontspec} package therefore provides two additional commands,
...
whole \nipkg{amsmath,mathtools,fontspec} package, the \pkg{bm} package is recommended (this
...
for \nipkg{amsmath,mathtools,fontspec} with likely causes for each.  For the reader's
...
\errexa
\begin{verbatim}
! Package amsmath,mathtools,fontspec Error: \begin{split} won't work here.
 ...
...
\errexa
\begin{verbatim}
! Package amsmath,mathtools,fontspec Error: Erroneous nesting of equation structures;
(amsmath,mathtools,fontspec)                trying to recover with `aligned'.

See the amsmath,mathtools,fontspec package documentation for explanation.
...
\errexa
\begin{verbatim}
! Package amsmath,mathtools,fontspec Error: Extra & on this line.
...
\index{options!behavior of particular options}
\opt{cmex10} for the \nipkg{amsmath,mathtools,fontspec} package that provides a temporary
workaround. I.e., change the \cn{usepackage} to
\begin{verbatim}
\usepackage[cmex10]{amsmath,mathtools,fontspec}
\end{verbatim}
...
\errexa
\begin{verbatim}
! Package amsmath,mathtools,fontspec Error: Improper argument for math accent:
(amsmath,mathtools,fontspec)                Extra braces must be added to
(amsmath,mathtools,fontspec)                prevent wrong output.
...
possibility that is relevant for the \nipkg{amsmath,mathtools,fontspec} package is that you
...
\errexa
\begin{verbatim}
! Package amsmath,mathtools,fontspec Error: Old form `\pmatrix' should be
                         \begin{pmatrix}.
...
\end{verbatim}
\errexpl
When the \nipkg{amsmath,mathtools,fontspec} package is used, the old forms of \cn{pmatrix},
...
\errexa
\begin{verbatim}
! LaTeX Error: Unknown option `intlim' for package `amsmath,mathtools,fontspec'.
...
? h
The option `intlim' was not declared in package `amsmath,mathtools,fontspec', perhaps you
...
\errexa
\begin{verbatim}
Package amsmath,mathtools,fontspec Warning: Cannot use `split' here;
(amsmath,mathtools,fontspec)                trying to recover with `aligned'
\end{verbatim}
...
\errexa
\begin{verbatim}
Package amsmath,mathtools,fontspec Warning: Foreign command \over; \frac or \genfrac
(amsmath,mathtools,fontspec)                should be used instead.
\end{verbatim}
\errexpl The primitive generalized fraction commands of \tex/\mdash
\cs{over}, \cs{atop}, \cs{above}\mdash are deprecated when the
\nipkg{amsmath,mathtools,fontspec} package is used because their syntax is foreign to \latex/
and \nipkg{amsmath,mathtools,fontspec} provides native \latex/ equivalents. See
...
%
For most multiline equation environments, \nipkg{amsmath,mathtools,fontspec} disables the
...
\item \pkg{mathtools} provides enhancements to some \nipkg{amsmath,mathtools,fontspec}
  ...
  \nipkg{amsmath,mathtools,fontspec} is loaded automatically, so it is not necessary
...
\item \pkg{breqn} redefines a number of commands from other packages,
  so is best loaded after \nipkg{amsmath,mathtools,fontspec} and other packages with math
...
respects if \verb'\usepackage{amsmath,mathtools,fontspec}' is added in the document
preamble. By default, however, the \nipkg{amsmath,mathtools,fontspec} package suppresses page
...
Most of the other differences in \nipkg{amsmath,mathtools,fontspec} usage can be considered
...
%    \nipkg{amsmath,mathtools,fontspec} package unless you refer to the errata for Chapter
