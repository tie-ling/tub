...
%%%   keywords  = latex, amsmath,mathtools,fontspec, examples, documentation
%%%   abstract  = This is a test file containing extensive examples of
%%%               mathematical constructs supported by the amsmath,mathtools,fontspec
%%%               package."
%%% ====================================================================
% convert \(...\) to \(\):
%  git ls-files -z | xargs -0I{} perl -pi -e 's/(\s)\\((.+?)\\)/\1\\(\2\\)/sg' {}
...
\usepackage{amsmath,mathtools,fontspec,amsthm,amssymb,parskip,interval,fontspec,mathtools}
...
\title{Sample Paper for the \pkg{amsmath,mathtools,fontspec} Package\\
...
\begin{document}
\maketitle
\markboth{Sample paper for the {\protect\ntt\lowercase{amsmath,mathtools,fontspec}} package}
...
\section{Various font features of the \pkg{amsmath,mathtools,fontspec} package}
\label{s:font}
\subsection{Bold versions of special symbols}

In the \pkg{amsmath,mathtools,fontspec} package \cn{boldsymbol} is used for getting
...
the \pkg{amsmath,mathtools,fontspec} package.  (Basic \LaTeX\ provides
...
In the \pkg{amsmath,mathtools,fontspec} package \cn{leftroot} and \cn{uproot} allow you to adjust
...
\verb=\lim=.
The \pkg{amsmath,mathtools,fontspec} package provides \cn{DeclareMathOperator} and
...
The following special operator names are predefined in the \pkg{amsmath,mathtools,fontspec}
...
In \pkg{amsmath,mathtools,fontspec} there are optional arguments \verb"t" and \verb"b" for
...
list of the \pkg{amsmath,mathtools,fontspec} package,
...
S{é}minaire Initiation {à} l'Analyse, G. Choquet, M. Rogalski, J.
Saint Raymond, at the Universit{é} Pierre et Marie Curie, Paris, 23e
ann{é}e.
